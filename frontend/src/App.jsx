...
function Landing() { const [id, setId] = useState(''); const navigate = useNavigate(); const submit = (event) => { event.preventDefault(); if (id.trim()) navigate(`/verify/${encodeURIComponent(id.trim())}`) }; return <div className="landing"><header className="public-nav"><Link className="brand" to="/"><span className="brand-mark">✦</span><span>Med<span className="brand-accent">guard</span></span></Link><div><Link className="plain-link" to="/login">Partner login</Link><Link className="button button-dark" to="/register">Join the network</Link></div></header><section className="hero"><div className="hero-copy"><p className="eyebrow">MEDICAL SUPPLY CHAIN INTELLIGENCE</p><h1>Know the story<br /><em>behind the medicine.</em></h1><p className="hero-lede">Trace every medical product from source to patient care. One verified history, made visible.</p><form className="lookup" onSubmit={submit}><span>⌕</span><input value={id} onChange={(event) => setId(event.target.value)} placeholder="Enter a batch or product ID" aria-label="Batch or product ID" /><button className="button button-dark">Trace origin <span>→</span></button></form><div className="hero-links"><Link to="/verify">Open verification center</Link><span>QR scanning supported in partner apps</span></div></div><div className="hero-visual"><div className="orbit orbit-one" /><div className="orbit orbit-two" /><div className="leaf-stamp">✣</div><div className="origin-note"><span className="pulse" /> live provenance<br /><strong>source to shelf</strong></div><div className="vertical-path"><span>🌿</span><i /><span>📦</span><i /><span>🧪</span><i /><span>💊</span></div></div></section><section className="principles"><div><span>01</span><strong>Source clarity</strong><p>Collection details and coordinates, recorded at origin.</p></div><div><span>02</span><strong>Custody confidence</strong><p>Every handoff carries an accountable timestamp.</p></div><div><span>03</span><strong>Public proof</strong><p>Scan a product and see the evidence, not a promise.</p></div></section><footer className="site-footer"><div><span className="footer-kicker">VANATRACE</span><p>Created by <strong>Omkar Tambe</strong></p></div><div className="social-links" aria-label="Social links"><a href="https://www.instagram.com/omkar24_46/" target="_blank" rel="noreferrer" aria-label="Instagram" title="Instagram"><svg viewBox="0 0 24 24" aria-hidden="true"><rect x="3" y="3" width="18" height="18" rx="5" /><circle cx="12" cy="12" r="4" /><circle cx="17.5" cy="6.5" r="1" className="social-dot" /></svg></a><a href="https://www.linkedin.com/in/omkar2446/" target="_blank" rel="noreferrer" aria-label="LinkedIn" title="LinkedIn"><svg viewBox="0 0 24 24" aria-hidden="true"><path d="M5 8v11M5 5.5v.1M10 19v-6a3 3 0 0 1 6 0v6M10 10v9M16 19v-6" /></svg></a><a href="https://github.com/omkar2446" target="_blank" rel="noreferrer" aria-label="GitHub" title="GitHub"><svg viewBox="0 0 24 24" aria-hidden="true"><path d="M9 19c-4 .9-4-2-5-2m10 4v-3.9c0-1 .1-1.4-.5-2.1 1.7-.2 3.5-.8 3.5-3.8 0-.8-.3-1.5-.8-2 .1-.2.3-1-.1-2.1 0 0-.7-.2-2.2.8a7.6 7.6 0 0 0-4 0c-1.5-1-2.2-.8-2.2-.8-.4 1.1-.2 1.9-.1 2.1-.5.5-.8 1.2-.8 2 0 3 1.8 3.6 3.5 3.8-.5.5-.5 1-.5 2.1V21" /></svg></a></div></footer></div> }
function AuthFrame({ title, subtitle, children }) { return <div className="auth-page"><Link className="brand" to="/"><span className="brand-mark">✦</span><span>Med<span className="brand-accent">guard</span></span></Link><div className="auth-card"><p className="eyebrow">PARTNER PORTAL</p><h1>{title}</h1><p>{subtitle}</p>{children}</div><small className="privacy-note">Sensitive identity documents remain private and are never included in public verification.</small></div> }
function Scanner({ onClose, onScan }) { const [message, setMessage] = useState('Starting camera...'); useEffect(() => { const scanner = new Html5Qrcode('qr-reader'); let active = true; scanner.start({ facingMode: 'environment' }, { fps: 10, qrbox: { width: 250, height: 180 } }, (decodedText) => { if (!active) return; active = false; onScan(decodedText) }, () => {}).then(() => setMessage('Point your camera at a Medguard QR or barcode')).catch(() => setMessage('Camera unavailable. Allow camera access or use manual entry.')); return () => { active = false; scanner.stop().catch(() => {}) } }, [onClose, onScan]); return <div className="scanner-backdrop" role="dialog" aria-modal="true" aria-label="Scan product"><div className="scanner-modal"><button className="close-button" onClick={onClose}>×</button><p className="eyebrow">CAMERA SCANNER</p><h2>Scan product</h2><p className="muted">{message}</p><div id="qr-reader" /><button className="button button-outline full" onClick={onClose}>Enter ID manually</button></div></div> }
function Login() { const [form, setForm] = useState({ email: '', password: '' }); const [error, setError] = useState(''); const navigate = useNavigate(); const submit = async (event) => { event.preventDefault(); setError(''); try { const data = await authService.login(form); authService.save(data); navigate('/dashboard') } catch (err) { setError(err.response?.data?.detail || 'Login could not be completed. Check your API connection.') } }; return <AuthFrame title="Welcome back" subtitle="Sign in to your Medguard workspace."><form className="auth-form" onSubmit={submit}>{error && <div className="alert alert-error">{error}</div>}<label>Email<input type="email" required value={form.email} onChange={(event) => setForm({ ...form, email: event.target.value })} /></label><label>Password<input type="password" required value={form.password} onChange={(event) => setForm({ ...form, password: event.target.value })} /></label><button className="button button-dark full">Sign in <span>→</span></button><p className="form-foot">New to Medguard? <Link to="/register">Create an account</Link></p></form></AuthFrame> }
function Register() {
...
function VerifyResult() {
	...
	useEffect(() => {
		verificationService.lookup(identifier)
			.then((response) => setData(response.data))
			.catch((err) => setError(err.response?.status === 404 ? 'No public record was found for this identifier.' : 'Verification service is unavailable.'))
	...
	const buildEvents = () => {
		...
		if (data.collection_date) {
			evs.push({ icon: '🌿', title: 'Collection', subtitle: data.collection_location || 'Origin Point', date: data.collection_date });
		}
		if (data.transfers) {
			data.transfers.forEach(t => evs.push({ icon: '📦', title: 'Custody Transfer', subtitle: `Quantity: ${t.quantity}`, date: t.date }));
		}
		if (data.processing) {
			data.processing.forEach(p => evs.push({ icon: '🏭', title: 'Processing', subtitle: `Input: ${p.input} → Output: ${p.output}`, date: p.date }));
		}
		if (data.lab_reports) {
			data.lab_reports.forEach(l => evs.push({ icon: '🧪', title: 'Laboratory Test', subtitle: `Status: ${l.result}`, date: l.date }));
		}
		if (data.manufacturing_date) {
			evs.push({ icon: '💊', title: 'Final Product', subtitle: data.product_name, date: data.manufacturing_date });
		}
	...
	return (
		<div className="center-page result-page">
			<div className="page-top">
				<div>
					<p className="eyebrow">TRACEABILITY RESULT</p>
					<h1>{identifier}</h1>
					<p className="muted">A public view of recorded provenance.</p>
				</div>
				<Link className="button button-outline" to="/verify">New lookup</Link>
			</div>

			{loading && <div className="loading-state">Loading verified history...</div>}
			{error && <div className="alert alert-error">{error} The backend is the source of truth; no journey is fabricated here.</div>}

			<div className="verification-banner">
				<span className="verified-mark">✓</span>
				<div>
					<small>PUBLIC RECORD STATUS</small>
					<h2>{data ? 'Verified Product' : 'Verification pending'}</h2>
					<p>{data?.product_name || 'Awaiting a backend traceability record for this identifier.'}</p>
				</div>
			</div>
			...
			<div className="journey">
				{events.map((event, index) => (
					...
					>
						<div className="stage-icon">{event.icon}</div>
						<div>
							<strong>{event.title}</strong>
							<small>{event.subtitle}</small>
							<span style={{ display: 'block', marginTop: '4px', fontSize: '0.75rem', color: '#4ade80' }}>
								{formatDateTime(event.date)}
							</span>
						</div>
						{index < events.length - 1 && <i className="journey-line" />}
					</motion.button>
				))}
			</div>
			...
			{selected && (
				<div className="detail-drawer">
					...
					<dl>
						...
						<dt>Date & Time</dt>
						<dd><strong>{formatDateTime(selected.date)}</strong></dd>
					</dl>
				</div>
			)}
			...
			<div className="result-grid">
				<section className="info-panel">
					<p className="eyebrow">SOURCE HERB</p>
					<h2>{data?.herb_name || 'Source data pending'}</h2>
					<div className="data-list">
						<span>Collection Location<strong>{data?.collection_location || 'Not recorded'}</strong></span>
						<span>Batch ID<strong>{data?.batch_id || 'Not recorded'}</strong></span>
						<span>Collection Date & Time<strong>{formatDateTime(data?.collection_date)}</strong></span>
						{data?.manufacturing_date && (
							<span>Manufacturing Date & Time<strong>{formatDateTime(data?.manufacturing_date)}</strong></span>
						)}
					</div>
				</section>
				<section className="info-panel blockchain-panel">
					<p className="eyebrow">BLOCKCHAIN VERIFICATION</p>
					<h2>BLOCKCHAIN VERIFIED ✓</h2>
					<p className="muted">Network: Hyperledger Fabric. Ledger Status: VERIFIED.</p>
				</section>
			</div>
		</div>
	);
...
function CollectorBatchCreate() {
	...
	const [form, setForm] = useState({
		...
		quantity: '',
		unit: 'kg',
		collection_date: defaultDateTime,
		collection_location: '',
		latitude: '',
		longitude: '',
		notes: ''
	...
	const useLocation = () => navigator.geolocation?.getCurrentPosition(
		(position) => setForm({ ...form, latitude: position.coords.latitude.toFixed(6), longitude: position.coords.longitude.toFixed(6) }),
	...
	if (created) return (
		<Shell>
			<div className="success-panel batch-success">
				<span>✓</span>
				<p className="eyebrow">BATCH CREATED</p>
				<h1>{created.batch_id}</h1>
				<p><strong>{created.herb_name}</strong> · {created.quantity} {created.unit}</p>
				<p className="muted">Collection Date & Time: <strong>{formatDateTime(created.collection_date)}</strong></p>
				<p className="muted">Source: {created.collection_location} · Status: {created.status}</p>
				<div className="success-actions">
					<Link className="button button-dark" to={`/batches/${created.batch_id}`}>View batch</Link>
					<Link className="button button-outline" to="/dashboard">Back to workspace</Link>
				</div>
			</div>
		</Shell>
	);
	...
	return (
		<Shell>
			<div className="workspace-head">
				<div>
					<p className="eyebrow">COLLECTOR WORKSPACE</p>
					<h1>Register original herb source</h1>
					<p className="muted">The system generates the Batch ID after your source record is saved.</p>
				</div>
				<Link className="button button-outline" to="/dashboard">← Workspace</Link>
			</div>
			<form className="panel batch-form" onSubmit={submit}>
				{error && <div className="alert alert-error">{error}</div>}
				<div className="form-grid">
					<label>Herb name<input required value={form.herb_name} onChange={(e) => update('herb_name', e.target.value)} placeholder="Ashwagandha" /></label>
					<label>Scientific name<input required value={form.scientific_name} onChange={(e) => update('scientific_name', e.target.value)} placeholder="Withania somnifera" /></label>
				</div>
				<div className="form-grid">
					<label>Quantity<input required type="number" min="0.01" step="0.01" value={form.quantity} onChange={(e) => update('quantity', e.target.value)} /></label>
					<label>Unit<select value={form.unit} onChange={(e) => update('unit', e.target.value)}><option>kg</option><option>g</option><option>tonnes</option></select></label>
				</div>
				<div className="form-grid">
					<label>Collection Date & Time<input required type="datetime-local" value={form.collection_date} onChange={(e) => update('collection_date', e.target.value)} /></label>
					<label>Collection location<input required value={form.collection_location} onChange={(e) => update('collection_location', e.target.value)} placeholder="Punjab" /></label>
				</div>
				<div className="location-fields">
					<label>Latitude<input type="number" min="-90" max="90" step="0.000001" value={form.latitude} onChange={(e) => update('latitude', e.target.value)} /></label>
					<label>Longitude<input type="number" min="-180" max="180" step="0.000001" value={form.longitude} onChange={(e) => update('longitude', e.target.value)} /></label>
					<button type="button" className="button button-outline" onClick={useLocation}>⌖ Use my current location</button>
				</div>
				<label>Source and collection notes<textarea rows="4" value={form.notes} onChange={(e) => update('notes', e.target.value)} placeholder="Describe the collection context" /></label>
				<button className="button button-dark">Create batch <span>→</span></button>
			</form>
		</Shell>
	);
}
					...
					<tbody>
						{batches.map(b => (
							<tr key={b.batch_id}>
								...
								<td>{formatDateTime(b.collection_date || b.created_at)}</td>
								<td><Status tone="neutral">{b.status}</Status></td>
								<td><Link className="button button-outline" to={`/batches/${b.batch_id}`}>View</Link></td>
							</tr>
						))}
					</tbody>
				</table>
...
function Detail({ type }) {
	...
	useEffect(() => {
		setLoading(true);
		setError('');
		const fetcher = type === 'batch' ? batchService.get(id) : productService.get(id);
		fetcher.then(res => setItem(res.data))
			.catch(err => setError(err.response?.data?.detail || `Failed to load ${type} record`))
			.finally(() => setLoading(false));
	}, [id, type]);
	...
	return (
		<Shell>
			<div className="workspace-head">
				<div>
					<p className="eyebrow">{type === 'batch' ? 'BATCH RECORD' : 'PRODUCT RECORD'}</p>
					<h1>{id}</h1>
					{item && <p className="muted">{type === 'batch' ? item.herb_name : item.name}</p>}
				</div>
				<Link className="button button-dark" to={`/verify/${id}`}>Trace history →</Link>
			</div>

			{loading && <div className="loading-state">Loading {type} details...</div>}
			{error && <div className="alert alert-error">{error}</div>}

			{item && (
				<div className="detail-layout" style={{ display: 'grid', gridTemplateColumns: '1fr 300px', gap: '24px' }}>
					<section className="panel">
						<h2>{type === 'batch' ? 'Herb Batch Provenance Overview' : 'Product Information'}</h2>
						<div className="data-list" style={{ marginTop: '16px', display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(200px, 1fr))', gap: '16px' }}>
							{type === 'batch' ? (
								<>
									<div><span>Herb Name</span><strong>{item.herb_name} ({item.scientific_name})</strong></div>
									<div><span>Available Quantity</span><strong style={{ color: '#4ade80' }}>{item.quantity} {item.unit}</strong></div>
									<div><span>Collection Location</span><strong>{item.collection_location}</strong></div>
									<div><span>Collection Date & Time</span><strong>{formatDateTime(item.collection_date)}</strong></div>
									<div><span>Registration Date & Time</span><strong>{formatDateTime(item.created_at)}</strong></div>
									<div><span>Current Status</span><Status tone="neutral">{item.status}</Status></div>
									{item.latitude && item.longitude && (
										<div><span>GPS Coordinates</span><strong>{item.latitude}, {item.longitude}</strong></div>
									)}
								</>
							) : (
								<>
									<div><span>Product Name</span><strong>{item.name}</strong></div>
									<div><span>Description</span><strong>{item.description || 'N/A'}</strong></div>
									<div><span>Manufactured Date & Time</span><strong>{formatDateTime(item.created_at)}</strong></div>
									<div><span>Manufacturer ID</span><strong>User #{item.manufacturer_id}</strong></div>
								</>
							)}
						</div>
					</section>
					...
					<section className="panel">
						...
						<div className="success-actions" style={{ display: 'flex', flexDirection: 'column', gap: '10px', marginTop: '16px' }}>
							{type === 'batch' && (
								<>
									<Link className="button button-dark full" to={`/batches/${id}/transfer`}>Transfer Custody</Link>
									{(role === 'PROCESSOR' || role === 'ADMIN') && (
										<Link className="button button-dark full" to={`/batches/${id}/process`}>Record Processing</Link>
									)}
									{(role === 'LABORATORY' || role === 'ADMIN') && (
										<Link className="button button-dark full" to={`/batches/${id}/test`}>Issue Lab Certificate</Link>
									)}
								</>
							)}
							<Link className="button button-outline full" to={`/verify/${id}`}>Public Provenance View</Link>
						</div>
					</section>
				</div>
			)}
